...
def main():
    ...
    while True:
    	comando = input("Digite comando: ") #vai receber o comando do teclado
    	if len(comando) == 0: #se o comando estiver vazio, acaba o loop
    		break
    	direcao = comando[:comando.index(" ")] #a direcao é substring do começo até o espaço
    	...
    	else: #se a partir do espaço há um input invalido, a qtdade dá invalida e a entrada é descartada
    		continue
    	if direcao == "CIMA": #foi pra cima, aumenta y
    		y += qtd
    	elif direcao == "BAIXO": #foi pra baixo, diminui y
    		y -= qtd
    	elif direcao == "ESQUERDA": #foi pra esquerda, diminui x
    		x -= qtd
    	elif direcao == "DIREITA": #foi pra direita, aumenta x
    		x += qtd
    	else: #direcao inválida, desconsidera a entrada
    		continue
